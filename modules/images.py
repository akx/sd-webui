    ...
    def resize(im, w, h):
        ...
        if scale > 1.0:
            try:
                ...
                upscaler = find_upscaler("Lanczos")
                logger.warning(f"Could not find upscaler {upscaler_name!r}, using {upscaler.name} instead.")
            im = upscale(im, scale=scale, upscaler=upscaler)
